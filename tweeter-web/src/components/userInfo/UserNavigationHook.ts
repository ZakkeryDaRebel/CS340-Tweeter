...
const { displayErrorMessage } = useMessageActions();
  ...
  return { navigateToUser: (event: React.MouseEvent) => navigateToUser(event, displayErrorMessage, displayedUser, authToken, setDisplayedUser, navigate) };
}



const navigateToUser = async (event: React.MouseEvent, displayErrorMessage: (message: string, bootstrapClasses?: string | undefined) => string, displayedUser: User | null, authToken: AuthToken | null, setDisplayedUser: (user: User) => void, navigate: NavigateFunction): Promise<void> => {
