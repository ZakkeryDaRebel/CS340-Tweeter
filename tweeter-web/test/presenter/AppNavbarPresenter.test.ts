...
describe("AppNavbarPresenter", () => {
  ...
  it("calls logout on the user service with the correct auth token", async () => {
    ...
    // verify is the same as doing these two lines of code
    // let [capturedAuthToken] = capture(mockService.logout).last();
    // expect(capturedAuthToken).toEqual(authToken);
  });

  it("the presenter tells the view to clear the info message that was displayed previously, clears the user info, and navigates to the login page", async () => {
    await appNavbarPresenter.logOut(authToken);
  ...
  it("the presenter tells the view to display an error message and does not tell it to clear the info message, clear the user info or navigate to the login page when unsuccessfull", async () => {
    let error = new Error("An error occurred");
    when(mockService.logout(anything())).thenThrow(error);

    await appNavbarPresenter.logOut(authToken);

    // following code is to debug the test
